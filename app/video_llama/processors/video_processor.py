...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, shortest_side=None, sampling="uniform", return_msg=False):
    ...
    vr = (
        VideoReader(uri=video_path, width=shortest_side)
        if shortest_side
        else VideoReader(uri=video_path, height=height, width=width)
    )
    vlen = len(vr)
    ...
    # Sample indices
    if sampling == "uniform":
        indices = np.linspace(0, vlen - 1, n_frms, dtype=np.int32)
    elif sampling == "headtail":
        half_frms = n_frms // 2
        indices_h = np.random.choice(range(vlen // 2), half_frms, replace=False)
        indices_t = np.random.choice(range(vlen // 2, vlen), half_frms, replace=False)
        indices = np.sort(np.concatenate((indices_h, indices_t)))
    else:
        raise NotImplementedError(f"Sampling method '{sampling}' is not implemented")
    ...
    # Calculate and format the message
    fps = float(vr.get_avg_fps())
    sec = ", ".join(str(round(x / fps, 1)) for x in indices)
