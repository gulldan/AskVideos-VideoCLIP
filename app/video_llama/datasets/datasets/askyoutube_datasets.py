...
class AskYoutubeDataset(BaseDataset):
    ...
    def _load_video_paths(self, vis_root):
        return {path.split("/")[-1]: path for path in glob.glob(os.path.join(vis_root, "*"))}
    ...
    def _load_annotations(self, ann_root):
        ...
        for captions_file in captions_files:
            ...
            for i, chunk in enumerate(captions):
                ...
                cap = ""
                for seg in chunk:
                    if "dDurationMs" not in seg or (seg.get("aAppend")):
                        continue
...
class AskYoutubeInstructDataset(BaseDataset):
    def __init__(
        ...
        self.video_paths = {path.split("/")[-1]: path for path in glob.glob(os.path.join(vis_root, "*"))}
        joined_captions = []
    ...
    def create_conversation_list(self, sample_dict, use_transcripts=True):
        ...
        if use_transcripts:
            ...
            prompt = "Answer the questions from the video:\n\n"
        if len(qa) == 0:
            # print('Empty QA')
            return []
        ...
        for _ in range(num_retries):
            ...
            try:
                ...
                sources = preprocess_multimodal(
                    copy.deepcopy(conversation_list), None, cur_token_len=self.num_video_query_token, msg=msg
                )
...
def convert_source_vicuna_format(sources):
    ...
    for source in sources:
        new_source = []
        for _i, sentence in enumerate(source):
...
def preprocess(
    ...
    return {"input_ids": input_ids, "labels": targets, "texts": texts}
